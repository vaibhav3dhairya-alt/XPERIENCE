...
# This version includes a fix for the vague query bug and creates a more
# seamless, conversational transition from AI search to personalization.
...
def get_intent_and_preferences_from_ai(user_query, context=""):
    ...
    Intents: 'greet', 'ask_for_recommendation', 'start_personalization', 'provide_preference', 'ask_for_surprise', 'ask_for_clarification'.
    ...
    Analyze the user query. If the query is too vague to determine a category (e.g., "suggest something"), set the intent to 'ask_for_clarification'.
    ...
    Response: {{"intent": "ask_for_clarification", "preferences": {{"category": null, "budget": null, "vibe": null, "group": null}}}}
    """
    try:
...
def handle_ongoing_conversation(from_number, message, session):
    ...
    if current_state == 'awaiting_freeform_query':
        ...
        if intent == 'ask_for_clarification':
            # The AI found the query too vague, so we pivot to the personalization flow
            session['state'] = 'awaiting_budget'
            user_sessions[from_number] = session
            send_text_reply(from_number, "I can help with that, but I need a few more details. Let's find the perfect spot. What's your budget? (e.g., cheap, moderate, or expensive)")
        else:
...
def send_recommendations(from_number, preferences):
    ...
    if not recommendations and preferences:
